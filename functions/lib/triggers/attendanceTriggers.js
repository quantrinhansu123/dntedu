...
exports.recalculateStudentStats = exports.onSessionComplete = exports.onAttendanceWrite = void 0;
...
function getDaysPerWeek(schedule) {
    if (!schedule)
        return 2; // Default 2 days per week
...
exports.onAttendanceWrite = functions
    ...
    const beforeRecord = change.before.exists ? change.before.data() : null;
    ...
    const daysPerWeek = getDaysPerWeek(classData === null || classData === void 0 ? void 0 : classData.schedule);
    ...
    for (const studentEntry of record.students) {
        ...
        const expectedEndDate = calculateExpectedEndDate(remaining, daysPerWeek, studentData.startDate || record.date);
        // Update student
        const updateData = {
 ...
 */
exports.onSessionComplete = functions
    .region(REGION)
    .firestore
    ...
    const absentStudents = attendance.students.filter(s => s.status === 'Vắng' || s.status === 'Có phép' || s.status === 'Nghỉ');
 ...
 */
exports.recalculateStudentStats = functions
    .region(REGION)
    .pubsub
    .schedule('0 2 * * *')
...
//# sourceMappingURL=attendanceTriggers.js.map
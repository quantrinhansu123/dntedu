/**
 * Script để chạy migration cho bảng assets
 * Chạy: npx tsx scripts/runAssetsMigration.ts
 */
...
async function showMigration() {
  try {
    ...
    const migrationPath = path.join(__dirname, '../supabase/migrations/030_create_assets_table.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf-8');

    console.log('📄 MIGRATION SQL CHO BẢNG ASSETS\n');
    ...
    console.log('✅ Sau khi chạy migration, bảng assets sẽ được tạo.\n');
    console.log('💡 Sau đó bạn có thể thử thêm tài sản lại trong ứng dụng.\n');
